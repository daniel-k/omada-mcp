...
const createSsidSchema = z.object({
    ...
    broadcast: coercedBoolean().optional().describe('Whether to broadcast the SSID name (default: true). Set false for hidden networks.'),
    guestNetEnable: coercedBoolean().optional().describe('Whether this is a guest network'),
    vlanEnable: coercedBoolean().optional().describe('Whether VLAN tagging is enabled'),
    vlanId: z.coerce.number().int().optional().describe('VLAN ID (1-4094). Required when vlanEnable is true.'),
    pskSetting: coercedObject(pskSettingSchema).optional().describe('WPA-Personal settings. Required when security is 3.'),
    mloEnable: coercedBoolean().optional().describe('Whether MLO (Multi-Link Operation) is enabled'),
    pmfMode: z.coerce.number().int().optional().describe('PMF mode: 1=Mandatory, 2=Capable, 3=Disable'),
    enable11r: coercedBoolean().optional().describe('Whether 802.11r fast roaming is enabled'),
