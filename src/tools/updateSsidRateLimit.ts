...
const customRateLimitSchema = z.object({
    downLimitEnable: z.boolean().describe('Whether to limit downlink speed'),
    downLimit: z.number().int().optional().describe('Downlink speed limit value (Kbps: 1-10485760, Mbps: 1-10240)'),
    downLimitType: z.number().int().optional().describe('Downlink unit: 0=Kbps, 1=Mbps'),
    upLimitEnable: z.boolean().describe('Whether to limit uplink speed'),
    upLimit: z.number().int().optional().describe('Uplink speed limit value (Kbps: 1-10485760, Mbps: 1-10240)'),
    upLimitType: z.number().int().optional().describe('Uplink unit: 0=Kbps, 1=Mbps'),
});
...
const rateLimitSettingSchema = z.object({
    ...
    customSetting: customRateLimitSchema.optional().describe('Custom rate limit settings'),
});
...
const updateSsidRateLimitSchema = z.object({
    ...
    clientRateLimit: rateLimitSettingSchema.optional().describe('Per-client rate limit settings'),
    ssidRateLimit: rateLimitSettingSchema.optional().describe('Per-SSID rate limit settings'),
